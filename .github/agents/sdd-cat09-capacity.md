...
|---------|---------|-------|------|-------|------|
| CAP-01 | 予測モデル構築・検証 | <Team> | D+3 | 高 | - |
| CAP-02 | 自動スケーリング設定 | <Team> | D+4 | 中 | CAP-01 |
| CAP-03 | ステージング環境での動作確認 | <Team> | D+5 | 高 | CAP-02 |
| CAP-04 | 本番環境への適用 | <Team> | D+6 | 高 | CAP-03 |
| CAP-05 | インフラチームトレーニング | <Team> | D+7 | 中 | CAP-04 |
```

**実行指示**:
1. 02_plan の予測モデル から逆算してタスク化
2. 依存関係を明示
3. 担当者不明な場合は <TBD> で記載
4. 期限は要件の期限から逆算
...
2. サブエージェント `sdd-code-generator-cat09` を起動
3. 出力：
...
2. サブエージェント `sdd-verifier-cat09` を起動
3. 実行内容：
	- テストケース生成（受入条件から逆算）
	- アプリ実行 + テスト検証
