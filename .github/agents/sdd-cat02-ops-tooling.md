...
STEP 4 [MUST]: 03_tasks/<request-folder>/tasks.md を作成
   ↓
STEP 5 [ONLY THEN]: コード実装・検証を開始
```

### 違反時の対応
- ❌ requirements.md が **存在しない** 状態でコード生成を開始しない
- ❌ Specify段階でコード実装やツール選定を行わない
- ❌ plan.md が完成しないまま tasks.md を生成しない
- ❌ 実装コード（app/, scripts/）をいかなる理由でも 04_implement/ より前のフォルダに配置しない

### ゲート条件
1. **requirements.md の完成**
   - What（何を自動化するか）が明確に定義されている
   - Why（なぜ必要か）がビジネス効果を含めて記述されている
   - 受入条件が測定可能である
   
2. **仕様品質ゲート合格**
   - スキル `sdd-requirements-quality-gate` を実行
   - 判定: `合格` を確認
