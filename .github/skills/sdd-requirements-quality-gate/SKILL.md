...
## 使用するタイミング
- `01_specify/requirements.md` を新規作成または更新したとき。
...
## 必須入力
- 対象ファイルパス（`categories/<category>/01_specify/requirements.md`）
- カテゴリ名と対象スコープ
...
2. 対象の `01_specify/requirements.md` を読む。
3. ルール1-5に対して評価する。
4. 出力契約に従って結果を返す。
5. Fail がある場合は、最小限の文面修正のみ提案する。

## すぐ使えるプロンプト例
- 「`sdd-requirements-quality-gate` を `categories/01_監視_モニタリング/01_specify/requirements.md` に実行して」
